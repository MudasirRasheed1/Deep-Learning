...
class HRNetW48(nn.Module):
    """
    HRNet-W48 for Semantic Segmentation with 5-channel input support
    """

    def __init__(self, num_classes=2, input_channels=5):
        super(HRNetW48, self).__init__()

        # Stage 1: Stem (Modified for 5-channel input)
        ...
        # Final layer (aggregate multi-resolution features)
        self.final_layer = nn.Sequential(
            nn.Conv2d(48 + 96 + 192 + 384, 256, 1, bias=False),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        feat1 = y_list[0]
        feat2 = F.interpolate(y_list[1], size=(h, w), mode='bilinear', align_corners=False)
        feat3 = F.interpolate(y_list[2], size=(h, w), mode='bilinear', align_corners=False)
        feat4 = F.interpolate(y_list[3], size=(h, w), mode='bilinear', align_corners=False)
...
def train_one_epoch(model, dataloader, criterion, optimizer, scaler, device, epoch):
    ...
    running_mscr_diff = 0.0  # Differentiable mSCR value
    running_acc = 0.0
    running_iou_land = 0.0
    running_iou_water = 0.0
    running_mscr_eval = 0.0  # Non-differentiable evaluation mSCR
    running_scr_k4 = 0.0
    running_scr_k8 = 0.0
    running_scr_k24 = 0.0
    running_scr_k48 = 0.0
    ...
    for batch_idx, (images, masks) in enumerate(pbar):
        ...
        if cfg.USE_MIXED_PRECISION:
            with autocast():
                ...
                loss, (ce_loss, dice_loss, mscr_loss, mscr_diff) = criterion(outputs, masks)
        ...
        if batch_idx % 50 == 0:
            logger.debug(f"Epoch {epoch+1} Batch {batch_idx}/{len(dataloader)} - "
                        f"Loss: {loss.item():.4f}, CE: {ce_loss:.4f}, Dice: {dice_loss:.4f}, "
                        f"mSCR_loss: {mscr_loss:.4f}, mSCR_diff: {mscr_diff:.4f}, mSCR_eval: {mscr_scores['mSCR']:.4f}")
...
def validate(model, dataloader, criterion, device, epoch):
    """Validate the model with detailed logging"""
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("HRNet-W48 with Hybrid Edge Channels Training on CAID Dataset")
    logger.info("Modified mSCR Evaluation (Per-k Averaging)")
    logger.info("=" * 80)
    ...
    # Create model
    logger.info("\nInitializing HRNet-W48 model with hybrid input...")
    model = HRNetW48(
    ...
    for epoch in range(cfg.NUM_EPOCHS):
        ...
        logger.info(f"    Land   - Train: {train_metrics['iou_land']:.4f} | Val: {val_metrics['iou_land']:.4f}")
        logger.info(f"    Water  - Train: {train_metrics['iou_water']:.4f} | Val: {val_metrics['iou_water']:.4f}")
        logger.info(f"\nmSCR SCORES:")
        logger.info(f"  Differentiable - Train: {train_metrics['mscr_diff']:.4f} | Val: {val_metrics['mscr_diff']:.4f}")
        logger.info(f"  Evaluation     - Train: {train_metrics['mscr_eval']:.4f} | Val: {val_metrics['mscr_eval']:.4f}")
        logger.info(f"\nSCR by K-values (Validation):")
        logger.info(f"  k=4  (Strict):  {val_metrics['scr_k4']:.4f}")
        logger.info(f"  k=8:            {val_metrics['scr_k8']:.4f}")
        logger.info(f"  k=24:           {val_metrics['scr_k24']:.4f}")
        logger.info(f"  k=48 (Lenient): {val_metrics['scr_k48']:.4f}")
